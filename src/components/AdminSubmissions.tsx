...
export default function AdminSubmissions() {
    ...
    try {
      ...
      if (photosData && !photosError) {
        photosData.forEach((photo) => {
          photoCounts[photo.submission_id] =
            (photoCounts[photo.submission_id] || 0) + 1;
        });
      }

      // Get questionnaire completion status
      const { data: questionnaireData, error: questionnaireError } =
        await supabase.from("vehicle_questionnaire").select("submission_id");

      const questionnaireCompleted = new Set();
      if (questionnaireData && !questionnaireError) {
        questionnaireData.forEach((q) =>
          questionnaireCompleted.add(q.submission_id)
        );
      }

      // Combine data
      const enrichedSubmissions: Submission[] = submissionsData.map(
        (submission) => ({
          ...submission,
          photos_count: photoCounts[submission.submission_id] || 0,
          questionnaire_completed: questionnaireCompleted.has(
  ...
  const applyFilters = () => {
    ...
    if (makeFilter !== "all") {
      filtered = filtered.filter(
        (submission) => submission.make === makeFilter
      );
    }
  ...
  const getUniqueValues = (key: keyof Submission): string[] => {
    return [...new Set(submissions.map((s) => String(s[key])).filter(Boolean))];
  };
      ...
      <div className="p-6 border-b border-imx-gray-200">
        ...
        {showFilters && (
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-imx-gray-50 rounded-lg">
            <div>
              <label className="block text-sm font-medium text-imx-gray-700 mb-2">
                Status
              </label>
            ...
            <tbody className="bg-white divide-y divide-imx-gray-200">
              {filteredSubmissions.map((submission) => (
                <tr
                  ...
                  className="hover:bg-imx-gray-50"
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      ...
                      <div className="text-sm text-imx-gray-500">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-imx-black">
                      {submission.vehicle_year} {submission.make}{" "}
                      {submission.model}
                    </div>
                    <div className="text-sm text-imx-gray-500">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-imx-gray-500">
                    <div className="flex items-center space-x-2">
                      ...
                      <span>•</span>
                      <span>
                        {submission.questionnaire_completed ? "Q✓" : "Q✗"}
                      </span>
                    </div>
                  </td>
